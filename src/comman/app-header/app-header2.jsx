...
const HeaderWithSidebar2 = () => {
  ...
  useEffect(() => {
    // 1️⃣ Check ALL possible scroll elements
    ...
    const findScroller = () => {
      possibleScrollElements.forEach((el) => {
        if (!el) return;
        if (el.scrollHeight > el.clientHeight + 20) {
          // this element can scroll
          activeScroller = el;
        }
      });

      if (!activeScroller) activeScroller = window;
    };
    ...
    // Attach listener
    const target =
      activeScroller === window ? window : activeScroller;
  ...
  //   window.addEventListener("wheel", handleScroll, { passive: true });       // desktop
  //   window.addEventListener("touchmove", handleScroll, { passive: true });  // mobile
